...
export const importExcel = async (filePath = 'data/Publicadores_Informes.xlsx') => {
    ...
    for (let tipo_Publicador of [ { descripcion: 'Publicador'}, { descripcion: 'Precursor regular' }, { descripcion: 'Precursor auxiliar' }]) {
        await runAsync(db, `INSERT OR IGNORE INTO Tipo_Publicador (descripcion) VALUES (?)`, [tipo_Publicador.descripcion]);
    }
    ...
    for (let p of jsonPublicadores) {
        let nombre = p.Nombre && p.Nombre.indexOf(',') !== -1 ? p.Nombre.substring(p.Nombre.indexOf(',') + 2) : p.Nombre;
        let apellidos = p.Nombre && p.Nombre.indexOf(',') !== -1 ? p.Nombre.substring(0, p.Nombre.indexOf(',')) : '';
        if(nombre == 'Total')
            continue; // Ignorar filas de totales
        ...
        let params = [
            nombre, apellidos, p['Fecha de nacimiento']?.toISOString().substring(0,10), p['Fecha de bautismo']?.toISOString().substring(0,10),
            p.Grupo, p['Sup. Grupo'] === "Sup" ? 1 : p['Sup. Grupo'] === "Aux" ? 2 : 0, p.Sexo?.substring(0, 1), privilegio?.id,
            tipo_publicador?.id, p.Ungido ? 1 : 0, p.Calle, p.Núm, p.Colonia,
            p['Teléfono fijo'], p['Teléfono móvil'], p['Contacto de emergencia'], p['Tel. Contacto de emergencia'],
            p['Correo Contacto de emergencia'], nombre, apellidos
        ];
        await runAsync(db, `INSERT OR IGNORE INTO Publicadores(
        ...
        where not exists (select 1 from Publicadores where nombre = ? and apellidos = ?)`, params);
    }
    ...
    for (let p of jsonInf) {
      let publicador = Publicadores.find(pub => (pub.apellidos ? pub.apellidos + ", " : "") + pub.nombre == p.Nombre);
      ...
      await runAsync(db, `INSERT OR IGNORE INTO Informes(
        id_publicador, mes, mes_enviado, predico_en_el_mes, cursos_biblicos, id_tipo_publicador, horas, notas, horas_SS
      ) select ?,?,?,?,?,?,?,?,?
      where not exists (select 1 from Informes where id_publicador = ? and mes = ?)`, params);
    }
