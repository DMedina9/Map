...
const fetchPublicadores = async () => {
    return await window.api.invoke('get-publicadores');
};

const addPublicador = async (publicador) => {
    return await window.api.invoke('add-publicador', publicador);
};

const updatePublicador = async (id, publicador) => {
    return await window.api.invoke('update-publicador', { id, ...publicador });
};

const deletePublicador = async (id) => {
    return await window.api.invoke('delete-publicador', id);
};
                ...
                <tbody>
                    {publicadores.map((pub) => (
                        <tr key={pub.id} className="border-b">
                            <td className="px-4 py-2">{pub.nombre} {pub.apellidos}</td>
